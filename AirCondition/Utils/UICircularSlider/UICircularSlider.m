...
- (void)setMinimumValue:(float)minimumValue {
	if (minimumValue != _minimumValue) {
		_minimumValue = minimumValue+0.35;
...
- (void)setMaximumValue:(float)maximumValue {
	if (maximumValue != _maximumValue) {
		_maximumValue = maximumValue+0.35;
...
- (void)drawThumbAtPoint:(CGPoint)sliderButtonCenterPoint inContext:(CGContextRef)context {
    /*
     图像
     */
    UIGraphicsBeginImageContext(CGSizeMake(kThumbRadius*4, kThumbRadius*4));
    CGContextRef imageCtx = UIGraphicsGetCurrentContext();
    [self.thumbTintColor setFill];
  	CGContextBeginPath(imageCtx);
    CGContextScaleCTM(imageCtx, 2, 2);
    CGContextMoveToPoint(imageCtx, 0, 0);
    CGContextAddArc(imageCtx, kThumbRadius, kThumbRadius, kThumbRadius, 0.0, 2*M_PI, NO);
    ...
    CGContextDrawImage(context, CGRectMake(sliderButtonCenterPoint.x-kThumbRadius, sliderButtonCenterPoint.y-kThumbRadius, kThumbRadius*2, kThumbRadius*2), image.CGImage);
...
- (void)panGestureHappened:(UIPanGestureRecognizer *)panGestureRecognizer {
	...
	switch (panGestureRecognizer.state) {
		case UIGestureRecognizerStateChanged: {
            ...
            CGFloat value = translateValueFromSourceIntervalToDestinationInterval(angle, 0, 2*M_PI, self.minimumValue, self.maximumValue);
            if(ABS(self.value - value) > (self.maximumValue - self.minimumValue)*0.5)
            {
...
{
    if(gestureRecognizer == self.gestureRecognizers[0])
    {
        CGPoint tapLocation = [gestureRecognizer locationInView:self];
        double ret = pow(tapLocation.x-self.frame.size.width/2, 2)+pow(tapLocation.y-self.frame.size.height/2, 2);
        return ret < pow(130, 2) && ret > pow(110, 2);
    }
    return YES;
}
